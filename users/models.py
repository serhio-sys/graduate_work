...
class NewUser(AbstractUser):
    ...
    weapon_equiped = models.ForeignKey('game.Weapon',verbose_name="WEAPON",default=None,null=True,blank=True,on_delete=models.SET_NULL, related_name='equipped_weapon_user')
    armor_equiped = models.ForeignKey('game.Armor',verbose_name="ARMOR",default=None,null=True,blank=True,on_delete=models.SET_NULL, related_name='equipped_armor_user')
    weapon2_equiped = models.ForeignKey('game.Weapon', verbose_name="SEC WEAPON", default=None,null=True,blank=True,on_delete=models.SET_NULL, related_name='equipped_weapon2_user')
    ...
    enemy = models.ForeignKey("game.Enemy",verbose_name="ENEMY",default=None,null=True,blank=True,on_delete=models.SET_NULL)
    ...
    def return_all_armor(self) -> int:
        bonus = settings.ROLES[self.role]['dmg']
        defence = self.defence + (self.agility*bonus)
    ...
    def get_summary_damage(self) -> dict:
        bonus = self.agility*settings.ROLES[self.role]['dmg']
        if self.weapon_equiped is None and self.weapon2_equiped is None:
    ...
    def get_summary_stats(self) -> dict:
        ...
        return {"str": {"hero": total_str, "sum": sum_str, "total": total_str + sum_str if total_str + sum_str >= 0 else 0},
    ...
    def CheckEXP(self):
        if self.lvl >= 99:
            ...
            if self.exp>=100:
                ...
                self.upgrade_points=self.upgrade_points+1
                self.exp=0
                self.save()
        return self.lvl
